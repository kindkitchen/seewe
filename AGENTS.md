...
- explore skills in ./skills/ to be ready to use any that match
...
- maintain a top-level `CHANGELOG.md`: newest first, grouped under
  `## YYYY-MM-DD` headings as `- <change>` bullets; create it on first change,
  add an entry with every change
- know the project's documentation sources (`README.md`, `CHANGELOG.md`,
  `docs/`, ...); update them together with the change itself so they never go
  stale

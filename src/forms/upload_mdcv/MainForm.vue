  ...
  <div class="flex flex-col">
    <FormKit type="form" id="upload-mdcv-form" @submit="handle_submit">
      <p v-show="is_user_with_nik" class="italic text-sm">
        Unfortunatelly you haven't define username for your account. That's why
        the CV naming option is not available for you. Your CV link will have
        auto-generated id.
      </p>
      <BlurGuardCmp
        :model-value="!is_user_with_nik"
        :relative_tw='tw("bottom-10 left-20")'
      >
        <template #guard>
          <t-btn
            v-show="!show_add_username_form"
            @click="() => (show_add_username_form = true)"
        ...
        <ModalCmp v-model="show_add_username_form">
          ...
          <template #buttons>
            <div class="flex flex-col">
              <AddNikForm
                @on_success="() => (show_add_username_form = false)"
              />
              <t-btn @click="() => (show_add_username_form = false)"
              >Cancel</t-btn>
            </div>
          </template>
        </ModalCmp>
